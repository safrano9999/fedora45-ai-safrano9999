...
Optional `WEBHOOK_URL` and `WEBHOOK_BEARER` enable webhook calls immediately after WELCOME in existing Safrano9999 fullrun schedules and startup routines. The existing `new`/`skip` setup repeats this optional group as `WEBHOOK_URL02`/`WEBHOOK_BEARER02`, etc. Empty URLs are skipped; an empty bearer sends the request without an Authorization header. The image sends a generic HTTP POST to each configured URL and delivers its response through the configured main Telegram account. Our version-check URL ends in `?--check`; removing those seven characters requests the normal full workflow. No n8n-specific payload is hardcoded in the generic webhook caller. Current versions use `✅`; available updates use `🟡`. The normal workflow, its gates and tagging semantics remain unchanged.
